...
func setup_title_screen() -> void:
	...
	new_game.grab_focus()
	new_game.mouse_entered.connect(play_audio.bind(button_focus))
	load_game.mouse_entered.connect(play_audio.bind(button_focus))
	exit.mouse_entered.connect(play_audio.bind(button_focus))

func start_game() -> void:
	play_audio(button_pressed)
	LevelManager.load_new_level(START_CUTSCENE, "", Vector2.ZERO)
